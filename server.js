...
const PENDING_ORDERS_PATH = path.join(DATA_DIR, 'pending_orders.json'); // 新增：待支付订单
...
const BOT_DATA_PATH = path.join(DATA_DIR, 'bot.json');
const AUTO_RENEWAL_PATH = path.join(DATA_DIR, 'auto_renewal.json'); // 新增：自动续费配置
...
// QQ机器人相关数据
let botData = { bindings: {}, pendingVerify: {} };
...
setInterval(() => {
    ...
    if (cleanedCaptcha > 0 || cleanedRateLimit > 0 || cleanedPending > 0) {
        logger.info(`🧹 清理了 ${cleanedCaptcha} 个过期验证码, ${cleanedRateLimit} 个速率限制记录, ${cleanedPending} 个待支付订单`);
    }
}, 5 * 60 * 1000);
...
function readJsonFile(filePath, defaultValue = {}) {
    ...
    try {
        ...
        if (typeof parsed !== 'object' || parsed === null) {
            logger.warn(`⚠️ 文件内容格式错误 ${filePath}，使用默认值`);
            return defaultValue;
        }
        return parsed;
    } catch (e) {
...
function loadLocalData() {
    ...
    logger.info(`已加载本地订单数据: ${Object.keys(localOrders).length} 条`);

    // 加载用户数据
    const usersData = readJsonFile(USERS_PATH, { users: [] });
    localUsers = usersData.users || [];
    logger.info(`已加载本地用户数据: ${localUsers.length} 个`);

    // 加载积分数据
    const pointsData = readJsonFile(POINTS_PATH, { points: {} });
    localPoints = pointsData.points || {};
    logger.info(`已加载本地积分数据: ${Object.keys(localPoints).length} 个用户`);

    // 加载已处理订单
    processedOrders = readJsonFile(PROCESSED_ORDERS_PATH, {});
    logger.info(`已加载已处理订单: ${Object.keys(processedOrders).length} 条`);
    ...
    // 加载待支付订单
    pendingOrders = readJsonFile(PENDING_ORDERS_PATH, {});
    // 清理过期的待支付订单（超过10分钟）
    const now = Date.now();
    let cleanedCount = 0;
    Object.keys(pendingOrders).forEach(orderId => {
        if (pendingOrders[orderId].expiresAt < now) {
    ...
    if (cleanedCount > 0) {
        savePendingOrders();
        logger.info(`已清理 ${cleanedCount} 个过期的待支付订单`);
    }
    logger.info(`已加载待支付订单: ${Object.keys(pendingOrders).length} 个`);

    // 加载签到数据
    checkinData = readJsonFile(CHECKIN_PATH, {});
    logger.info(`已加载签到数据: ${Object.keys(checkinData).length} 个用户`);
...
// 加载兑换码数据
    ...
    logger.info(`已加载服务器数据: ${Object.keys(serversData).length} 个`);

    // 初始化保存，确保所有数据文件存在
    if (!fs.existsSync(ORDERS_PATH)) {
        writeJsonFile(ORDERS_PATH, { lastSync: null, orders: {} });
        logger.info('📝 已创建初始文件: orders.json');
    }
    if (!fs.existsSync(USERS_PATH)) {
        writeJsonFile(USERS_PATH, { lastSync: new Date().toISOString(), users: [] });
        logger.info('📝 已创建初始文件: users.json');
    }
    if (!fs.existsSync(POINTS_PATH)) {
        writeJsonFile(POINTS_PATH, { lastSync: new Date().toISOString(), points: {} });
        logger.info('📝 已创建初始文件: points.json');
    }
    if (!fs.existsSync(PROCESSED_ORDERS_PATH)) {
        writeJsonFile(PROCESSED_ORDERS_PATH, {});
        logger.info('📝 已创建初始文件: processed_orders.json');
    }
    ...
    if (!fs.existsSync(PENDING_ORDERS_PATH)) {
        writeJsonFile(PENDING_ORDERS_PATH, {});
        logger.info('📝 已创建初始文件: pending_orders.json');
    }
    if (!fs.existsSync(CHECKIN_PATH)) {
        writeJsonFile(CHECKIN_PATH, {});
        logger.info('📝 已创建初始文件: checkin.json');
    }
    ...
    if (!fs.existsSync(SERVERS_PATH)) {
        writeJsonFile(SERVERS_PATH, { lastUpdate: new Date().toISOString(), servers: {} });
        logger.info('📝 已创建初始文件: servers.json');
    }
    ...
    // 初始化 bot.json（QQ机器人数据）
    const BOT_DATA_PATH = path.join(DATA_DIR, 'bot.json');
    if (!fs.existsSync(BOT_DATA_PATH)) {
        writeJsonFile(BOT_DATA_PATH, { bindings: {}, pendingVerify: {} });
        logger.info('📝 已创建初始文件: bot.json');
    }
    
    // 加载bot数据
    botData = readJsonFile(BOT_DATA_PATH, { bindings: {}, pendingVerify: {} });
    logger.info(`已加载QQ机器人数据: ${Object.keys(botData.bindings || {}).length} 个绑定`);
    
    // 加载自动续费数据
    autoRenewalData = readJsonFile(AUTO_RENEWAL_PATH, {});
    logger.info(`已加载自动续费配置: ${Object.keys(autoRenewalData).length} 个用户`);
...
const serverManager = {
    ...
    async createServer(username, planId, serverName = '', daemonId = null, imageId = null, customConfig = null) {
        ...
        try {
            ...
            console.log(`MCSManager实例创建成功: ${instanceUuid} (${plan.name}) - 用户: ${username}, 消耗积分: ${plan.points}`);
            ...
            if (afterPurchaseCommand) {
                ...
                exec(afterPurchaseCommand, (error, stdout, stderr) => {
                    if (error) {
                        console.error(`执行购买后命令失败: ${error.message}`);
                    } else {
...
function saveAutoRenewal() {
    return writeJsonFile(AUTO_RENEWAL_PATH, autoRenewalData);
}
...
function readBotData() {
    try {
        ...
        if (!parsed.pendingVerify) parsed.pendingVerify = {};
        
        return parsed;
    } catch (e) {
        logger.error('读取bot数据失败:', e.message);
        return { bindings: {}, pendingVerify: {} };
    }
}
...
function writeBotData(data) {
    try {
        fs.writeFileSync(BOT_DATA_PATH, JSON.stringify(data, null, 2), 'utf-8');
        return true;
    } catch (e) {
        logger.error('写入bot数据失败:', e.message);
        return false;
    }
}
...
function cleanExpiredVerify() {
    ...
    if (cleaned > 0) {
        ...
        logger.info(`清理了 ${cleaned} 个过期验证码`);
    }
}
...
function saveProcessedPayments() {
    return writeJsonFile(PROCESSED_PAYMENTS_PATH, processedPayments);
}

// 保存待支付订单
function savePendingOrders() {
    return writeJsonFile(PENDING_ORDERS_PATH, pendingOrders);
}
...
const requestHandler = async (req, res) => {
    ...
    try {
                ...
                if (action === 'get' && req.method === 'GET') {
                    ...
                    if (section === 'rootAdmin') {
                        ...
                        if (newPassword && newPassword.length > 0) {
                            ...
                            // 对新密码进行 bcrypt 加密
                            currentConfig.rootAdmin.password = await bcrypt.hash(newPassword, 10);
                            currentConfig.rootAdmin.passwordSet = true; // 标记密码已设置
                        }
                    } else if (section === 'mcsm') {
        ...
        // 查询用户实例列表（用于QQ机器人）
        if (pathname === '/api/mcsm/user/instances' && req.method === 'GET') {
            const username = query.username;
            
            if (!username) {
                jsonResponse(res, 400, { success: false, error: '缺少username参数' });
                return;
            }
        ...
        if (pathname.startsWith('/admin/') || pathname === '/admin') {
            ...
            const isPublicFile = publicFiles.includes(filePath);
            ...
            // 特殊处理：如果请求的是 /admin/js/marked.min.js，从 public/admin/js 目录提供
            if (filePath === '/js/marked.min.js') {
                const markedPath = path.join(__dirname, 'public/admin/js/marked.min.js');
                serveStaticFile(res, markedPath);
                return;
            }
...
function startServer() {
  ...
  |_|  |_|   \\_/\\_/   | .__/ 
                      |_|    
                `);
                logger.info(`服务已启动! (HTTPS) 访问 https://${HOST}:${PORT}`);
            });
        } catch (e) {
            // 增强：打印完整的错误对象以方便调试
  ...
  |_|  |_|   \\_/\\_/   | .__/ 
                      |_|    
                `);
                logger.info(`服务已启动! (HTTP) 访问 http://${HOST}:${PORT}`);
            });
        }
    } else {
        server = http.createServer(requestHandler);
