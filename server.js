...
const captcha = require('svg-captcha'); // 导入 svg-captcha 库
...
function calculateCustomPlanPrice(customConfig) {
    ...
    // cpuPercent is not directly limited here, but cpuCores could be.
...
const requestHandler = async (req, res) => {
    ...
    try {
        ...
        if (pathname === '/api/instance/pre-update' && req.method === 'POST') {
            try {
                const data = await parseBody(req);
                const { daemonId, uuid, memory, ports } = data;
                ...
                const beforePoints = calculateCustomPlanPrice({
                ...
                const afterPoints = calculateCustomPlanPrice({
                    memory: newMemory,
                    cpu: beforeCpuCores,
                    disk: beforeDiskGB,
                    ports: newPorts
                ...
                const pointDifference = beforePoints - afterPoints;
                let cost = 0;
                let refund = 0;
                let action = 'none';

                if (pointDifference > 0) { // 降级
                    refund = Math.floor(pointDifference * 0.9); // 90% 退款
                    action = 'downgrade';
                } else if (pointDifference < 0) { // 升级
                    cost = Math.ceil(Math.abs(pointDifference));
                    action = 'upgrade';
                }
                ...
                jsonResponse(res, 500, { status: 'error', error: '预计算失败: ' + e.message });
            }
            return;
        }
        ...
        if (pathname === '/api/instance/configure' && req.method === 'POST') {
            try {
                ...
                const oldConfig = {
                    ...
                    cpu: (currentDockerConfig.cpuUsage || 100) / 100,
                    disk: currentDockerConfig.maxSpace ? currentDockerConfig.maxSpace / 1024 : 10,
                ...
                const newConfigForCalc = {
                    ...
                    cpu: cpu ? cpu / 100 : oldConfig.cpu,
                    disk: disk ? disk / 1024 : oldConfig.disk,
                    ports: ports ? ports.join(',') : oldConfig.ports
                };
                ...
                if (cpu) updatePayload.docker.cpuUsage = cpu;
                if (disk) updatePayload.docker.maxSpace = disk;
                ...
                if (updateResult.data.status === 200) {
                    ...
                    if (pointsChange !== 0) {
                        ...
                        const previousPoints = pointsManager.getBalance(username);
                        // 回滚操作与原操作相反
                        localPoints[username].totalPoints = round(previousPoints - pointsChange);
                        if (!localPoints[username].deductHistory) localPoints[username].deductHistory = [];
                        localPoints[username].deductHistory.push({
                            points: pointsChange, // 正数表示扣除, 负数表示增加
                            reason: `配置更新失败退款 (实例: ${uuid.substring(0, 8)})`,
                            time: new Date().toISOString(),
                            previousPoints: previousPoints,
                            afterPoints: localPoints[username].totalPoints,
                            type: 'refund'
