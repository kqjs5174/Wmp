...
createApp({
    setup() {
        ...
        const fullConfig = computed({
            get() {
                if (config.value) {
                    return jsyaml.dump(config.value, { indent: 2 });
                }
                return '';
            },
            set(value) {
                try {
                    config.value = jsyaml.load(value);
                    showStatus('YAML 格式正确', 'success');
                } catch (e) {
                    showStatus('YAML 格式错误: ' + e.message, 'error');
                }
            }
        });
        ...
        const loadConfig = async () => {
            ...
            try {
                ...
                // 初始化 customPlan 及其嵌套属性
                if (!loadedConfig.customPlan) {
                    loadedConfig.customPlan = {
                        enabled: false,
                        defaultDuration: 30,
                        limits: {
                            minMemory: 512,
                    ...
                    // 确保嵌套对象存在
                    if (!loadedConfig.customPlan.limits) loadedConfig.customPlan.limits = {};
                    if (!loadedConfig.customPlan.pointsFormula) loadedConfig.customPlan.pointsFormula = {};
                }
        ...
        watch(currentView, (newView) => {
            if (newView === 'full') {
                // 当切换到完整配置视图时，确保 fullConfig 重新计算
                // 这将触发 get() 方法，将当前 config.value 转换为 YAML 字符串
                fullConfig.value = jsyaml.dump(config.value, { indent: 2 });
            }
        });
        ...
        const loadCoupons = async () => {
            const token = localStorage.getItem('rootToken');
            if (!token) {
                couponError.value = '未授权的访问: 未找到登录凭证';
                return;
            }
            ...
            try {
                ...
                if (result.code === 0) {
                    ...
                    throw new Error(result.msg || '加载兑换码失败');
                }
            } catch (error) {
        ...
        const updateUserPoints = async () => {
            ...
            const { username } = editingUser.value;
            ...
            try {
                if (type === 'set') {
                    ...
                    options.body = JSON.stringify({ username, points: value, reason: reason || '管理员设置积分' });
                } else if (type === 'add') {
                    url = `/api/points/add?username=${encodeURIComponent(username)}&points=${value}&reason=${encodeURIComponent(reason || '管理员增加积分')}`;
                    options.method = 'GET';
                } else { // deduct
                    url = `/api/points/deduct?username=${encodeURIComponent(username)}&points=${value}&reason=${encodeURIComponent(reason || '管理员扣减积分')}`;
                    options.method = 'GET';
                }
                ...
                showStatus(`用户 ${username} 的积分已更新。`, 'success');
            ...
            saveAnnouncement: async () => {
                ...
                try {
                    const response = await fetch('/api/announcement', {
                        ...
                        body: JSON.stringify({ content: announcement.value.content })
                    });
                    const result = await response.json();
                    if (result.code === 0) {
                        alert('公告保存成功！');
                    } else {
                        alert('保存公告失败: ' + result.msg);
                    }
                } catch (error) {
