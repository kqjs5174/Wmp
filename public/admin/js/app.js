...
/**
 * 计算实例价格
 * @param {object} config - { memory, cpu, disk, ports }
 * @returns {Promise<number>} 价格（积分）
 */
async function calculateInstancePrice(config) {
    try {
...
async function updateConfigurePricePreview() {
    ...
    // 2. 计算当前配置价值
    const currentConfig = {
        memory: parseInt(document.getElementById('configure-instance-memory').dataset.originalValue),
        cpu: parseFloat(document.getElementById('configure-instance-cpu').dataset.originalValue),
        disk: parseFloat(document.getElementById('configure-instance-disk').dataset.originalValue),
        ports: document.getElementById('configure-instance-ports').dataset.originalValue.split(',').length
    };
    const currentPrice = await calculateInstancePrice(currentConfig);
    currentInstancePrice = currentPrice; // Store globally
    if (currentPrice === -1) {
        currentPriceEl.textContent = '计算失败';
        return;
    }
    currentPriceEl.textContent = `${currentPrice} 积分`; // 移除 /天
    ...
    const newConfig = {
        ...
        ports: document.getElementById('configure-instance-ports').value.split(',').filter(p => p.trim()).length
    };
    const newPrice = await calculateInstancePrice(newConfig);
    if (newPrice === -1) {
        newPriceEl.textContent = '计算失败';
        return;
    }
    newPriceEl.textContent = `${newPrice} 积分`; // 移除 /天
    ...
    if (diff > 0) { // 升级
        ...
        diffPriceEl.textContent = `${diff.toFixed(2)} 积分`;
        diffPriceEl.style.color = '#f97316'; // Orange for increase
        if (userBalance < diff) {
            warningEl.textContent = `您的积分不足以支付升级费用 ${diff.toFixed(2)} 积分。`;
            warningEl.style.display = 'block';
            saveBtn.disabled = true;
        }
    } else if (diff < 0) { // 降级
        const refund = Math.abs(diff) * 0.9;
        diffLabelEl.textContent = '预计返还:';
        diffPriceEl.textContent = `${refund.toFixed(2)} 积分 (已扣除10%手续费)`;
...
async function showConfigureInstanceModal(uuid) {
    ...
    try {
        ...
        portsInput.dataset.originalValue = portNumbers.join(', ');
...
async function handleUpdateConfiguration() {
    ...
    const newConfig = { memory: memoryNum, cpu: cpuValue, disk: diskValue, ports: ports.length };
    const newPrice = await calculateInstancePrice(newConfig);
    const diff = newPrice - currentInstancePrice;

    // Confirmation dialog
    let confirmMessage = `确定要保存这些配置更改吗？<br><br>
        当前配置价值: <strong>${currentInstancePrice} 积分</strong><br>
        新配置价值: <strong>${newPrice} 积分</strong>`;
    
    if (diff > 0) {
        confirmMessage += `<br><br><span style="color:#ef4444;">此操作将立即从您的余额中扣除 <strong>${diff.toFixed(2)}</strong> 积分。</span>`;
    } else if (diff < 0) {
        const refund = Math.abs(diff) * 0.9;
        confirmMessage += `<br><br><span style="color:#10b981;">此操作将立即向您的余额返还 <strong>${refund.toFixed(2)}</strong> 积分 (已扣除10%手续费)。</span>`;
    }
    ...
    try {
        ...
        const response = await fetch(`${CONFIG.RENEWAL_API_URL}/api/instance/configure`, {
            ...
            body: JSON.stringify({
                ...
                username: currentUser.username,
                memory: memoryNum,
                cpu: cpuValue * 100, // Convert to cpuUsage
                disk: diskValue * 1024, // Convert to MB
                ports,
            })
        });
        ...
        if (result.code === 0) {
            closeConfigureInstanceModal();
            let pointsChangeText = result.data.pointsChange > 0 ? `+${result.data.pointsChange.toFixed(2)}` : result.data.pointsChange.toFixed(2);
            await showAlert(
                `配置更新成功！<br>
