...
async function loadDashboardData() {
    ...
    if (result.success) {
        ...
        showError('recent-orders-list', result.error);
    }
}
...
async function loadOrdersPage() {
    ...
    if (result.success) {
        ...
        showError('orders-list', result.error);
    }
}
...
function renderInstancesList() {
    ...
    let html = `
        <div class="search-result-header">
            <span>🖥️ 您的实例列表（共 <strong>${instancesData.length}</strong> 个）</span>
        </div>
    `;
    
    html += createInstancesTable(instancesData);
    ...
    instances.forEach(instance => {
        ...
        html += `
            <tr>
                ...
                <td>
                    <span class="expire-time ${expireInfo.class}">
                        ${expireInfo.text}
                    </span>
                </td>
                <td>
                    <button class="action-btn renew" onclick="renewInstance('${instance.daemonId}', '${instance.uuid}', '${instance.nickname}')">
                        ⏰ 续费
                    </button>
                </td>
            </tr>
...
async function loadServerPlans() {
    ...
    container.innerHTML = '<p class="loading">加载套餐中...</p>';
    
    try {
        const response = await fetch(CONFIG.ENDPOINTS.SERVER_PLANS);
        const result = await response.json();
        
        if (result.code === 0 && result.data && result.data.plans) {
            serverPlansData = result.data.plans;
            renderServerPlans();
        } else {
            container.innerHTML = `<div class="empty-state"><div class="icon">📦</div><p>${result.msg || '暂无套餐'}</p></div>`;
        }
    } catch (error) {
...
function renderServerPlans() {
    ...
    const planIds = Object.keys(serverPlansData);
    ...
    planIds.forEach(planId => {
        ...
        html += `
            <div class="plan-card ${planId}" data-plan-id="${planId}" onclick="selectPlan('${planId}')">
                <div class="plan-header">
                    <div class="plan-name">${plan.name || planId}</div>
                    <div class="plan-price">${plan.points} <span>积分</span></div>
                    <div class="plan-duration">${specs.duration || 30} 天</div>
                </div>
                <div class="plan-specs">
                    <div class="spec-item">
                        <span class="spec-label">CPU</span>
                        <span class="spec-value">${specs.cpu || 1} 核</span>
                    </div>
                    <div class="spec-item">
                        <span class="spec-label">内存</span>
                        <span class="spec-value">${specs.memory || '1G'}</span>
                    </div>
                    <div class="spec-item">
                        <span class="spec-label">存储</span>
                        <span class="spec-value">${specs.storage || '10G'}</span>
                    </div>
                </div>
            </div>
        `;
    ...
    // 添加空白套餐框（敬请期待）
    const emptyPlanNames = ['套餐 A', '套餐 B', '套餐 C', '套餐 D'];
    emptyPlanNames.forEach(name => {
        html += `
            <div class="plan-card empty-plan" style="opacity:0.6;cursor:not-allowed;">
                <div class="plan-header">
                    <div class="plan-name">${name}</div>
                    <div class="plan-price">-- <span>积分</span></div>
                    <div class="plan-duration">敬请期待</div>
                </div>
                <div class="plan-specs" style="color:#999;">
                    <div class="spec-item">
                        <span class="spec-label">CPU</span>
